...
class SCORE:
    ...
    def getPageCodeOfAllScores(self, StuId, Id):
        ...
        postdata = urllib.urlencode({
            ...
            'drop_xn': '',
            'drop_xq': '',
            'drop_type': '全部成绩'.decode('gbk').encode('utf-8'),
            'Button_cjcx': '查询'.decode('gbk').encode('utf-8'),
    ...
    def getPageCodeOfConverting(self):
        ...
        post_data = urllib.urlencode({'TextBox1': '',
                                      ...
                                      'drop_xq': '',
                                      'drop_type': '全部成绩'.decode('gbk').encode('utf-8'),
                                      'btn_xhxm': '学号姓名转换'.decode('gbk').encode('utf-8'),
                                      'hid_dqszj': ''
                                      })
        try:
    ...
    def getPageCodeOfPostingNameOrId(self, NameOrId):
        ...
        if NameOrId.isdigit():
            post_data = urllib.urlencode({
                ...
                'drop_xq': '',
                'drop_type': '全部成绩'.decode('gbk').encode('utf-8'),
                'Button2': '查询'.decode('gbk').encode('utf-8'),
                'hid_dqszj': ''
            })
        else:
            post_data = urllib.urlencode({
                ...
                'drop_xq': '',
                'drop_type': '全部成绩'.decode('gbk').encode('utf-8'),
                'Button3': '查询'.decode('gbk').encode('utf-8'),
                'hid_dqszj': ''
            })
        try:
            request = urllib2.Request(self.url, post_data)
    ...
    def printAllScores(self, StuId, Id):
        ...
        soup = BeautifulSoup(pageCode)
        p = re.compile('共找到(.*?)条'.decode('gbk').encode('utf-8'), re.S)
        ...
        for i in range(numberOfRecord):
            ...
            if i == 0:
                print '==========================', TermDown, '============================='
                if items[0][5] == '必修课'.decode('gbk').encode('utf-8') or items[0][5] == '任选课'.decode('gbk').encode('utf-8'):
                ...
                # 本条和上一条相同,学分绩求和
                if items[i][5] == '必修课'.decode('gbk').encode('utf-8') or items[i][5] == '任选课'.decode('gbk').encode('utf-8'):
            ...
            print ("%30s%20s%20s") % (items[i][3].decode('utf-8').encode('gbk'), items[i][4].decode('utf-8').encode('gbk'), items[i][8].decode('utf-8').encode('gbk'))
        # 打印各个学期的学分绩
        for i in list:
            print '\n============',i[0], '平均学分绩:', i[1] / i[2],'================'
    ...
    def getAveScore(self, StuID):
        soup = BeautifulSoup(self.getPageCodeOfInit())
        post_data = urllib.urlencode({'TextBox1': StuID,
                                      ...
                                      'drop_xq': '',
                                      'drop_type': '全部成绩'.decode('gbk').encode('utf-8'),
                                      'Button_xfj': '第一专业平均学分绩'.decode('gbk').encode('utf-8'),
                                      'hid_dqszj': '',
                                      }
                                     )
        try:
    ...
    def printStuInfo(self, NameOrId):
        ...
        if len(items) == 0:
            ...
            print '学号:', items[0][0].decode('utf-8').encode('gbk'), '\t', '姓名:', items[0][1].decode('utf-8').encode('gbk'), '\t', '性别:', items[0][2].decode('utf-8').encode('gbk'), '\t', '学院:', \
                items[0][3].decode('utf-8').encode('gbk'), '\t', '专业:', items[0][4].decode('utf-8').encode('gbk'), '\t', '班级:', items[0][5].decode('utf-8').encode('gbk'), \
                '\n平均学分绩:', self.getAveScore(items[0][0])
            print '==============================================================================='

        elif len(items) <= 100:
            for item in items:
                print '学号:', item[0].decode('utf-8').encode('gbk'), '\t', '姓名:', item[1].decode('utf-8').encode('gbk'), '\t', \
                    '性别:', item[2].decode('utf-8').encode('gbk'), '\t\t', '学院:', item[3].decode('utf-8').encode('gbk'), '\t', \
                    '专业:', item[4].decode('utf-8').encode('gbk'), '\t', '班级:', item[5].decode('utf-8').encode('gbk')
                print '平均学分绩:', self.getAveScore(item[0].decode('utf-8').encode('gbk'))
                print '==============================================================================='
        else:
    ...
    def start(self,NameOrId):
        ...
        if NameOrId == 'h':
            ...
            self.start(NameOrId.decode('gbk').encode('utf-8'))
        # 退出，同时输出版权信息
        elif NameOrId == 'q':
            self.exit()
        # 输出学生信息
    ...
    def exit(self):
        ...
        REPORTING BUGS:
            Report bugs to vennchao@gmail.com
...
print """
        ...
        E-mail: vennchao@gmail.com\n
        """     
# NameOrId由用户输入，在windows下是gbk编码的，所以要转换成utf-8编码在内部作为数据传送
NameOrId = raw_input('请输入姓名或学号(q退出,h帮助):')
ahut.start(NameOrId.decode('gbk').encode('utf-8'))
# 解决Windows命令提示符界面出现一闪而过的问题
os.system('pause')
